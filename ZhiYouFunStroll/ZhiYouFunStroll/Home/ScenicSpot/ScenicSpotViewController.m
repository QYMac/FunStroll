...
@interface ScenicSpotViewController ()<GeneralWaterfallFlowLayoutDelegate,UICollectionViewDelegate,UICollectionViewDataSource>

@property (nonatomic,strong) UICollectionView *scenicSpotCollectionView;
...
{
    [self.view addSubview:self.scenicSpotCollectionView];
    [self.scenicSpotCollectionView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)cartoonContentRefresh{
    ...
    // 设置刷新控件
    self.scenicSpotCollectionView.mj_header = header;
}

- (void)MJRefreshFooter{
    // 上拉刷新
    self.scenicSpotCollectionView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        //停止下拉刷新
        [self.scenicSpotCollectionView.mj_header endRefreshing];
        [self.scenicSpotCollectionView.mj_footer endRefreshing];
    }];
}
...
#pragma  mark - 懒加载
- (UICollectionView *)scenicSpotCollectionView {
    if (!_scenicSpotCollectionView) {
        GeneralWaterfallFlowLayout *layout = [[GeneralWaterfallFlowLayout alloc] init];
        _scenicSpotCollectionView = [[UICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        _scenicSpotCollectionView.delegate = self;
        _scenicSpotCollectionView.dataSource = self;
        //_scenicSpotCollectionView.scrollEnabled = NO;//禁止cell滑动
        _scenicSpotCollectionView.showsVerticalScrollIndicator = NO;
        _scenicSpotCollectionView.showsHorizontalScrollIndicator = NO;
        [_scenicSpotCollectionView registerClass:[CommunityCollectionViewCell class] forCellWithReuseIdentifier:@"cell"];
        _scenicSpotCollectionView.backgroundColor = RGB(231, 231, 231);
    }
    return _scenicSpotCollectionView;
}
