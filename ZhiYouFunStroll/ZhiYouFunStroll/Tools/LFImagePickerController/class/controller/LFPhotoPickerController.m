...
@property (nonatomic, strong) LFPhotoPreViewColletion *preViewColletionView;
...
- (void)viewWillLayoutSubviews
{
    ...
    if (self.preViewColletionView.hidden == NO) {
        WeakSelf
        collectionViewRect.size.height -= 100;
//        [UIView animateWithDuration:0.3 animations:^{
//            weakSelf.collectionView.frame = collectionViewRect;
...
//        [cell addInteraction:interaction];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected, LFAsset *cellModel, LFAssetCell *weakCell) {
        ...
        // 底部选中展示的视图
        [weakSelf setupPreViewColletionViewIndex:indexPath.row];
    };
    return cell;
}
...
- (void)setupPreViewColletionViewIndex:(NSInteger)index{
    ...
    CGRect collectionViewRect = [self viewFrameWithoutNavigation];
    collectionViewRect.size.height -= (toolbarHeight+100);
    if (@available(iOS 11.0, *)) {
        collectionViewRect.origin.x += self.view.safeAreaInsets.left;
        collectionViewRect.size.width -= self.view.safeAreaInsets.left + self.view.safeAreaInsets.right;
    }
    ...
    if (imagePickerVc.selectedModels.count <= 0) {
        self.preViewColletionView.hidden = YES;
    } else {
        self.preViewColletionView.hidden = NO;
        self.preViewColletionView.selectedModels = imagePickerVc.selectedModels;
    }
    
    self.preViewColletionView.frame = CGRectMake(0, collectionViewRect.origin.y + collectionViewRect.size.height, kWidth, 100);
    [self.view insertSubview:self.preViewColletionView atIndex:9999];
    
    WeakSelf
    // 延迟执行，确保布局已完成
    dispatch_async(dispatch_get_main_queue(), ^{
        NSIndexPath *targetIndexPath = [NSIndexPath indexPathForItem:index inSection:0];
        [weakSelf.collectionView scrollToItemAtIndexPath:targetIndexPath
...
- (LFPhotoPreViewColletion *)preViewColletionView{
    if (!_preViewColletionView) {
        _preViewColletionView = [[LFPhotoPreViewColletion alloc] init];
        _preViewColletionView.hidden = YES;
    }
    return _preViewColletionView;
}
