...
#import "SwitchPageView.h"
#import "UIView+QQFrame.h"
#import "QYBadgeLabel.h"

static const NSInteger itemTag = 100;

@interface SwitchPageView ()<UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource>
...
- (void)changeItemStatus:(NSInteger)index
{
    ...
    if ([self.delegate respondsToSelector:@selector(SwitchPageView:didSelectIndex:)]) {
        [self.delegate SwitchPageView:self didSelectIndex:index];
    }
}
...
- (void)setLineHeight:(CGFloat)lineHeight
{
    ...
    self.lineView.layer.cornerRadius = lineHeight/2;
}
...
- (UIScrollView *)titleScroll
{
    if (!_titleScroll) {
        ...
        if (self.contentTitles.count == 2) {
            _titleScroll.scrollEnabled = NO;
            _titleScroll.frame = CGRectMake(kWidth/4, 0, kWidth/2, self.titleViewHeight);
        }else{
            _titleScroll.frame = CGRectMake(0, 0, kWidth, self.titleViewHeight);
        }
